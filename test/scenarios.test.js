...
import { feedLifecycle } from '../scenarios/feed-lifecycle.js';
...
test('resolveEvents is deterministic: same base -> identical events, ids derived from the scenario', () => {
  const a = resolveEvents(feedLifecycle, BASE);
  const b = resolveEvents(feedLifecycle, BASE);
  assert.deepStrictEqual(a, b);
  assert.ok(a.every((ev) => ev.id.startsWith(`${feedLifecycle.id}_`)));
  assert.ok(a.every((ev) => Number.isFinite(ev.ts)));
  // offsets are relative to base: first event at base + its offset
  assert.equal(a[0].ts, (BASE + feedLifecycle.events[0].t) * 1000);
});
...
test('feed-lifecycle: replayed PURE (fold, no store), every assertion passes', () => {
  const state = replayPure(feedLifecycle, BASE);
  const results = runAssertions(feedLifecycle, state, BASE);
...
test('feed-lifecycle: replayed ON the memory substrate (real write path), same assertions pass', () => {
  const state = replayOnMemory(feedLifecycle, BASE);
  const results = runAssertions(feedLifecycle, state, BASE);
...
test('substrate replay and pure replay converge on identical observables', () => {
  const viaSubstrate = runAssertions(feedLifecycle, replayOnMemory(feedLifecycle, BASE), BASE);
  const viaPure = runAssertions(feedLifecycle, replayPure(feedLifecycle, BASE), BASE);
...
test('a scenario event that violates the schema refuses at replay with words', () => {
  const bad = { id: 'bad-sc', description: 'x', events: [{ t: 0, actor: 'u_a', type: 'post.created', payload: { feedId: 'f', format: 'text', title: 'no id' } }], assertions: [] };
  assert.throws(() => replayPure(bad, BASE), /missing required feed: id/);
});
