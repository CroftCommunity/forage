...
export function createLens({ session = null, transport = fetch, hiddenUris = new Set(), graphSource = null } = {}) {
  ...
  return {
    ...
    // Forget the remembered graph and the scopes derived from it — sign-out,
    // account switch, or an explicit refresh. The graph belongs to an account,
    // not to a device.
    forgetRings() { ringCache.clear(); graphCache.clear(); },

    // The members a scope narrows to, for the display filter. Uncapped, and
    // there is no longer a capped variant to distinguish it from: RING_CAP
    // bounded the ring BOARD's one-request-per-member fan-out, and that board
    // was retired with the /r/<rung> routes (2026-09-03). Filtering fans out
    // nothing, so nothing here needs a bound.
    ...
    async followWrites(ops, verb, onChunk) {
      if (!session) throw new Error(`lens: ${verb} needs a session — sign in first`);
      ...
      for (const writes of chunks) {
        ...
        try {
          ...
          const why = /HTTP 429/.test(e.message)
            ? 'HTTP 429 — the account\'s write budget for this hour is spent; try again later'
            : e.message.replace(/^lens: [^ ]+( all)? failed /, '');
          const err = new Error(`lens: ${verb} stopped after ${done} of ${total} — ${why}`);
          err.done = done; err.total = total;
          throw err;
        }
    ...
    async followAll(dids, { via, now = new Date().toISOString() } = {}) {
      ...
      try {
        await this.followWrites(ops, 'follow all', (writes, results) => {
    ...
    async unfollowAll(followUris) {
      ...
      let done = 0;
      await this.followWrites(ops, 'unfollow all', (writes) => { done += writes.length; });
      return done;
    },
