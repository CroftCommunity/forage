...
export const LEDGER = [
  ...
  { id: 'DL-028', kind: 'tolerance', capability: 'feed-ranking', tier: 'wide', label: '"Top" means two different things by board kind',
    ...
    date: '2026-08-26', status: 'active' },
  { id: 'DL-030', kind: 'tolerance', capability: 'feeds', tier: 'wide', label: 'Adoption signals come from a second read host',
    description: 'The share count and starter-pack count on a feed card come from constellation.microcosm.blue, an atproto-wide backlink index — NOT from the AppView, which exposes neither (ADR-003, amending ADR-002 for this narrow case). Bounded by construction: counts on feed generators only, never an intake path; degrade to ABSENT rather than to zero, so a host that is down removes the line instead of claiming nobody shared the feed; no viewer identity is ever sent, because the read goes through the plain transport and never the session. Windows are decoded from the backlink rkeys themselves (TIDs carry a microsecond timestamp), so a window costs no extra request.',
    reason: 'The question has no other answer: joining is private (DL-029) and no app.bsky endpoint counts quotes-of-a-feed or pack inclusions.',
