...
export const LEDGER = [
  ...
  { id: 'DL-027', kind: 'frontier', capability: 'compose', tier: 'wide', label: 'Composing is text-only, and mentions are not resolved',
    description: '3w publishes app.bsky.feed.post records with byte-indexed facets for #hashtags and bare http(s) links, both limits enforced from the lexicon (300 graphemes AND 3000 utf-8 bytes — a family emoji is 1 grapheme and 25 bytes, so the byte cap is reachable under the character cap). NOT built: images/video/gallery embeds; @mention facets (they need a resolveHandle per mention, and a mention that silently renders as plain text is worse than none, so they are left unfaceted and the composer says nothing about them); self-labels (content warnings); quote-posting from the composer; editing or deleting a post (the lens creates and unlikes — it has no delete for posts, deliberately). A post appears on the writer\'s Bluesky account immediately but may lag in a hashtag board, because those ride search indexing — the composer does not refetch the board and does not pretend the post is there.',
    date: '2026-08-26', status: 'open' },
