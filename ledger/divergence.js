...
export const LEDGER = [
  ...
  { id: 'DL-011', kind: 'tolerance', capability: 'voting', tier: 'wide',
    description: 'RETIRED 2026-08-27 (plan 2026-08-27-1). Was: lens scores are likes-only — ups = likeCount, downs = 0; bury counts do not exist on the network and controversial ranking is undefined at this tier.',
    reason: 'Bluesky has no downvote; pretending otherwise would fabricate data.',
    tolerance: 'score fields present, downs pinned 0',
    retired: 'The DIVERGENCE this excused no longer exists: the memory sandbox dropped downvotes too (owner, 2026-08-27), so both populations now agree that a vote is a boost or nothing. A tolerance is a standing permission for two tiers to disagree; when they stop disagreeing it is not satisfied, it is unnecessary. Kept as a record rather than deleted — the ledger is a history, and this row is the reason `downs` used to exist in shapes that no longer carry it.',
    date: '2026-08-25', retiredDate: '2026-08-27', status: 'retired' },
  ...
  { id: 'DL-013', kind: 'frontier', capability: 'voting', tier: 'wide', label: 'Boost = like (write)',
    description: 'SHIPPED 2026-08-25 (plan 2026-08-25-1, 3c): boosting a lens post creates a real app.bsky.feed.like via the OAuth session; unboost deletes it by exact rkey. The invariant scan names this pair as the lens\' ONLY writes. (The sentence "bury remains without an analogue" was true until 2026-08-27 and is now stale in the other direction: there is no bury to lack an analogue for.)',
  ...
  { id: 'DL-026', kind: 'tolerance', capability: 'feeds', tier: 'wide', label: 'Content language is Forage-local, because the account has none',
    description: 'VERIFIED 2026-08-26 against the official lexicons and a live probe. (a) app.bsky.feed.post.langs EXISTS — array, maxLength 3, items format "language" — so a post SELF-DECLARES its language; a getPosts probe returned record.langs=["en"]. (b) app.bsky.feed.searchPosts accepts a `lang` parameter ("Filter to posts in the given language... server may override language detection"), so search-backed boards could filter server-side. (c) app.bsky.actor.defs contains NO language preference: the complete def list is adultContentPref, contentLabelPref, savedFeedsPref, savedFeedsPrefV2, personalDetailsPref, declaredAgePref, feedViewPref, threadViewPref, interestsPref, mutedWordsPref, hiddenPostsPref, labelersPref, bskyAppStatePref, postInteractionSettingsPref, verificationPrefs — and the string "lang" appears nowhere in the file. CONSEQUENCE: the official app\'s "content languages" setting is app-local, not account state. Forage can neither read nor honour it, so Forage\'s language preference is its own, device-local, and the profile panel says so. Divergence from the mirror-the-account principle, accepted because the account has nothing to mirror. CONFIRMED POSITIVELY 2026-08-26 in the official client (bluesky-social/social-app, which drives bsky.app): `contentLanguages` lives in src/state/persisted/schema.ts — the client\'s own persisted state, not account state — so this entry no longer rests on the absence of a lexicon def. That file also settles two details we had guessed right (tags are \'BCP-47 2-letter language codes without region\', and postLanguage defaults to deviceLanguageCodes[0]) and names two places forage differs ON PURPOSE: the official client defaults contentLanguages to the device languages (forage defaults to NO filter — narrowing what you see unasked is the opposite of the point) and falls back to \'en\' when the device says nothing (forage says nothing rather than claim a language it does not know). Not built here: social-app\'s per-post language selector (postLanguage + postLanguageHistory in the composer). Revisit if atproto adds a language pref.',
    date: '2026-08-26', status: 'open' },

  { id: 'DL-027', kind: 'frontier', capability: 'compose', tier: 'wide', label: 'Composing is text-only, and mentions are not resolved',
    description: '3w publishes app.bsky.feed.post records with byte-indexed facets for #hashtags and bare http(s) links, both limits enforced from the lexicon (300 graphemes AND 3000 utf-8 bytes — a family emoji is 1 grapheme and 25 bytes, so the byte cap is reachable under the character cap). Images SHIPPED in phase 3 (2026-08-26): up to 4, alt text required (the server refuses a missing one and would silently accept a blank one), size gated client-side because uploadBlob returns 200 for an oversized blob and only createRecord refuses it, aspectRatio derived from the preview so a reader\'s feed does not jump as each image loads. NOT built: video/gallery embeds, external link cards; @mention facets (they need a resolveHandle per mention, and a mention that silently renders as plain text is worse than none, so they are left unfaceted and the composer says nothing about them); self-labels (content warnings); quote-posting from the composer; EDITING a post (that is putRecord, which the lens still does not do — the only way to change a post is to delete it and write another, which is what the network itself does). Deleting your own post SHIPPED in phase 2 (2026-08-26), guarded by a parsed-uri ownership check so a delete can never address another repo. A post appears on the writer\'s Bluesky account immediately but may lag in a hashtag board, because those ride search indexing — the composer does not refetch the board and does not pretend the post is there.',
    date: '2026-08-26', status: 'open' },

  { id: 'DL-037', kind: 'tolerance', capability: 'feeds', tier: 'mock', label: 'A sandbox feed is governed; a Bluesky feed is not',
    description: 'Both populations now call the place posts live a FEED (rename 2026-08-26, owner-directed: the sandbox term was "Field"). The rename went through the data contract, not just the labels — event types (field.created → feed.created), payload keys (fieldId → feedId), and the lexicon id (fyi.forage.field → fyi.forage.feed). Pre-1.0, so no migration layer: a log written before the rename cannot be folded, and store.legacyLog() detects it and refuses rather than folding it into an empty world, because the reducer has no default branch and silence would look identical to a first visit. Records already written to the live PDS under fyi.forage.field are orphaned by design. THE DIVERGENCE THE WORD NOW HIDES: a sandbox feed has membership, stewards and a public audit log; a Bluesky feed has none of those and cannot be posted into at all (DL-025). One word, two governance models — accepted because the shape is genuinely the same and the governance difference is stated wherever it matters. Phase 5 restores membership to a feed in the private BBS. Also renamed to avoid the collision this created: the ranked-listing selector and view (sel.feed/feedView) became sel.board/boardView, matching the vocabulary the lens already used.',
    date: '2026-08-26', status: 'open' },
  ...
  { id: 'DL-035', kind: 'tolerance', capability: 'moderation', tier: 'wide', label: 'A logged-out visitor gets a strict label floor, because there is no account to mirror',
    description: 'Signed in, moderation is the ACCOUNT\'s: Forage mirrors adultContentPref and contentLabelPref and adds no switch of its own (the piggy-back principle, D10) — including the account\'s choice to turn things ON. Logged out there is nothing to mirror, so instead of a permissive default a guest gets a fixed floor that hides unconditionally: porn, sexual, nudity, sexual-figurative, graphic-media, gore, self-harm, torture, corpse, !hide, !takedown, !warn. Modelled on bluebird\'s label floor (CroftC/bluebird/src/feed/labels.ts) and carrying its three rules: HIDE rather than blur-with-reveal (a tap-to-reveal control is a decoy door — it presents the material as one gesture away while pretending to withhold it), honour negated labels as retractions, and read the AUTHOR\'s labels as well as the item\'s. Applies to posts and to feed generators alike.',
    reason: 'The honest answer to "what would this visitor have chosen" is the safe thing, not everything.',
    tolerance: 'guest-tier visibility is narrower than any signed-in tier; not compared against memory',
    date: '2026-08-26', status: 'active' },
  { id: 'DL-036', kind: 'tolerance', capability: 'moderation', tier: 'wide', label: 'Muted content is ABSENT, never present-with-a-label',
    description: 'A muted word or muted account removes the post from boards, search and threads entirely — the same disappearance a blocked author already got. Forage previously rendered a placeholder row reading "[muted — matches your muted words]", which defeated the mute twice: the row still cost the reader a line of attention, and it announced exactly what was being withheld. Muting is CLIENT-SIDE RENDERING GUIDANCE — the account said "do not show me this" — and the only rendering that honours it is nothing at all. In a thread the node disappears with its subtree, exactly as a blocked author\'s already does. Divergence from bsky.app, which shows an expandable muted placeholder in threads.',
    reason: 'Owner, 2026-08-26: "it should just be absent — it\'s client side rendering guidance and we are confusing it in that case."',
    tolerance: 'muted items absent rather than masked; thread reply counts differ from bsky.app by the muted subtree',
    date: '2026-08-26', status: 'active' },
  { id: 'DL-031', kind: 'tolerance', capability: 'feeds', tier: 'wide', label: 'Discovery ordering is Forage-local',
    ...
    date: '2026-08-26', status: 'active' },
  { id: 'DL-032', kind: 'tolerance', capability: 'feed-ranking', tier: 'wide', label: '"Top" means two different things by board kind',
    ...
    date: '2026-08-26', status: 'active' },
  { id: 'DL-034', kind: 'tolerance', capability: 'feeds', tier: 'wide', label: 'Adoption signals come from a second read host',
    description: 'The share count and starter-pack count on a feed card come from constellation.microcosm.blue, an atproto-wide backlink index — NOT from the AppView, which exposes neither (ADR-004, amending ADR-002 for this narrow case). Bounded by construction: counts on feed generators only, never an intake path; degrade to ABSENT rather than to zero, so a host that is down removes the line instead of claiming nobody shared the feed; no viewer identity is ever sent, because the read goes through the plain transport and never the session. Windows are decoded from the backlink rkeys themselves (TIDs carry a microsecond timestamp), so a window costs no extra request.',
    reason: 'The question has no other answer: joining is private (DL-033) and no app.bsky endpoint counts quotes-of-a-feed or pack inclusions.',
    tolerance: 'adoption fields absent at every tier but wide, and absent at wide when the host does not answer',
    date: '2026-08-26', status: 'active' },
  { id: 'DL-033', kind: 'frontier', capability: 'feeds', tier: 'wide', label: 'Feed adoption is unmeasurable',
