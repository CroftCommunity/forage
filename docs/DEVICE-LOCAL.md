...
| `forage.alttext` | localStorage | the alt-text display preference (Advanced on /me) | `undecided` | a reading preference — one of the batch that a single preferences record would carry; decide the batch together |
...
| `forage.boardview` | localStorage | board density (compact / comfortable) | `undecided` | a reading preference; the preferences-record batch |
| `forage.cardsize` | localStorage | the card size notch | `undecided` | a reading preference; the preferences-record batch |
...
| `forage.gifautoplay` | localStorage | whether GIFs play on their own | `undecided` | a reading preference; the preferences-record batch |
| `forage.haptics` | localStorage | Buzz on like | `device` | a hardware feel; the phone that has a motor decides, and a laptop has none |
| `forage.hashtagsections` | localStorage | which hashtag sections show in the nav | `undecided` | a reading preference; the preferences-record batch |
...
| `forage.langs` | localStorage | the content-language filter | `undecided` | a reading preference; the preferences-record batch — note Bluesky's own `contentLanguages` preference exists and may be the half |
...
| `forage.pictures` | localStorage | how many pictures a post shows before folding into a carousel | `undecided` | a reading preference; the preferences-record batch |
| `forage.providermark` | localStorage | the atmo-provider mark beside handles, on or off | `undecided` | a reading preference; the preferences-record batch |
| `forage.rail` | localStorage | the right rail's panel order, or off | `undecided` | a reading preference; the preferences-record batch |
| `forage.ringexempt` | localStorage | ring scope: boards exempt from the scope | `undecided` | ring scope is a way of reading the account's graph; whether it follows the reader is the same question as the preferences batch, decided with it |
...
| `forage.skin` | localStorage | the chosen skin (light and dark are skins too — ADR-003) | `undecided` | a reading preference; the preferences-record batch — the one most readers would expect to follow them |
...
| `forage.threadflatten` | localStorage | thread shape: flatten | `undecided` | a reading preference; the preferences-record batch |
| `forage.threadfold` | localStorage | thread shape: fold depth | `undecided` | a reading preference; the preferences-record batch |
...
2. **`undecided` rows are batched.** Most are reading preferences; the natural half is ONE
   preferences record, decided once for all of them (and checked against Bluesky's own
