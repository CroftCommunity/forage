...
| `forage.feedindex.pds` | localStorage | the last-read `fyi.forage.feedindex` record from this account and the file it names, with its age (plan 2026-09-21 own-index-on-the-pds, Phase 3) | `cache` | the record on the atmo provider account is the truth; this is the copy that answers offline and the age the status line shows |
| `forage.feedindex` | localStorage | the reader's own discovery index and the mode (Add / Replace / Off) — plan 2026-09-08 feed-index Phase 2b | `record` | `fyi.forage.feedindex` at `self` — the file as a blob in the repo, or an https link, plus the mode (plan `plans/2026-09-21-plan-own-index-on-the-pds.md`, built 2026-09-21). Local is still the default; publishing MOVES the file out of this store (D6); Off stays here (D4) |
