...
- ~~**The reel's count line over a bright frame.**~~ — DONE 2026-09-25: a denser scrim and a
  shadow on the words (`.reel-bar`, `.reel-count`); `view-clip-fol` re-captured.
- ~~**An ended clip sits at its last frame.**~~ — DECIDED 2026-09-25 by the owner: **it loops.**
  The reel's player mounts with `loop`; a row's clip still plays once. Held by the journey.
...
- **A native mirror of drafts (feed-row O8).** Drafts live in this browser (`js/drafts.js`).
  ...
  (CLAUDE.md § External APIs), then a Settings choice. (No phone needed — not a device-queue item.)
