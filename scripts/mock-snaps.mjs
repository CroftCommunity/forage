...
for (const [name, vp] of Object.entries(VIEWPORTS)) {
  ...
  // ...RESPONSES first: the signed-in account's preferences, mutes and profile
  // (mock-thread's fixtures) — without them the moderation posture fails to
  // load and every frame carries a red toast that is not the mock's subject.
  const FI = {
    ...RESPONSES,
    'resolveHandle': { did: 'did:plc:mockpack' },
    ...
    // one fresh post, so the popular row reads LIVE under the default "Hide
    // inactive" (an empty feed would hide the only Bluesky-listed row)
    ...
    'getStarterPack?': { starterPack: { uri: PACK, cid: 'bafymock',
      ...
      // no label on the fixture: every label a guest floor knows HIDES the pack outright (DL-035),
      // and the frame is the head card, not the hidden state — the labelled rows are in the
      // feeds-index frames, from the real index
      joinedAllTimeCount: 12045, joinedWeekCount: 37, list: { uri: 'at://did:plc:mockpack/app.bsky.graph.list/l1', listItemCount: 14930 }, labels: [],
  ...
  // Exact waits, so a frame is never the skeleton (`.card.skeleton`) or an
  ...
  for (const route of FEED_INDEX_ROUTES) {
    ...
    try {
      ...
      // say what the page held instead — a timeout with no page is a guess
